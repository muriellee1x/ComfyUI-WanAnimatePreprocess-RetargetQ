...
def padding_resize(img_ori, height=512, width=512, padding_color=(0, 0, 0), interpolation=cv2.INTER_LINEAR, lowestY=-1.0):
    ...
    # 如果提供了lowestY参数且有效，进行裁切和贴底处理
    if lowestY > 0:
        # 判断lowestY是归一化坐标还是像素坐标
        # 如果 <= 1.0，认为是归一化坐标（推荐使用）；否则是像素坐标
        if lowestY <= 1.0:
            # 归一化坐标，转换为当前图像的像素坐标
            lowestY_int = int(lowestY * ori_height)
        else:
            # 像素坐标，直接使用
            lowestY_int = int(lowestY)
        
        # 确保lowestY在有效范围内
        if lowestY_int > 0 and lowestY_int <= ori_height:
            use_crop_mode = True
    
    # 如果使用裁切模式，执行裁切和贴底逻辑
    if use_crop_mode:
        # 裁切图像：保留从顶部到lowestY的部分
        img_cropped = img_ori[0:lowestY_int, :, :]
        cropped_height = lowestY_int
        cropped_width = ori_width
        ...
        # 缩放图像
        img_resized = cv2.resize(img_cropped, (new_width, new_height), interpolation=interpolation)
        if len(img_resized.shape) == 2:
            img_resized = img_resized[:, :, np.newaxis]
        
        # 贴底并水平居中
        y_offset = height - new_height  # 贴底：从底部向上放置
        x_offset = (width - new_width) // 2  # 水平居中
        
        img_pad[y_offset:y_offset + new_height, x_offset:x_offset + new_width, :] = img_resized
        
        return img_pad
    
    # 原始逻辑：没有lowestY参数时的处理
    img_pad = np.zeros((height, width, channel), dtype=img_ori.dtype)
    if channel == 1:
        img_pad[:, :, 0] = padding_color[0]
    else:
...
#     # Place image at bottom, centered horizontally
#     y_pad = height - new_height  # Bottom alignment
#     x_pad = (width - new_width) // 2  # Horizontal centering
#     img_pad[y_pad:y_pad + new_height, x_pad:x_pad + new_width, :] = img
